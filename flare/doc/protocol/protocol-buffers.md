...
- [`accept_request_raw_bytes`](../../rpc/rpc_options.proto)：
  - 这个Protocol Buffers的`option`用于修饰`.proto`中定义的方法。如果指定了，那么框架不会进行请求的反序列化（或合法性检查），而是直接将原始的二进制通过`RpcServerController::GetRequestRawBytes`传递给服务实现方。
  - *注意：该选项尚未实现，目前在`rpc_options.proto`中处于注释状态，`std_protocol.cc`中相关的实现也仍是`TODO`。*
...
- [`RpcServerController`](../../rpc/rpc_server_controller.h)：
  - `GetAcceptableCompressionAlgorithms` / `GetPreferredCompressionAlgorithm`：查询客户端能接受的、以及框架推荐的压缩算法。
  - `SetCompressionAlgorithm`：服务端开启压缩，对响应的消息主体使用指定算法进行压缩；通常配合上述查询接口使用。
  - `SetResponseAttachmentPrecompressed`：如果响应`attachment`已经按照`SetCompressionAlgorithm`指定的算法预先压缩过，可通过此方法告知框架避免再次压缩。
...
- `flare.max_queueing_delay_ms`：指定请求的最大排队时长，单位毫秒。如果设置且不为0，那么当系统负载较高时，如果这个方法的请求在系统内排队时间超过了这个限制，那么框架会对这个请求直接返回`rpc::STATUS_OVERLOADED`，不会调用具体的方法实现。
